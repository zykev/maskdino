...
@suppress_stdout()
def sam_load(ckpt_path: str) -> torch.nn:
    sam = sam_model_registry["vit_tiny"](checkpoint=ckpt_path)
    return sam


@torch.no_grad()
def sam_predict(sam, boxes_xyxy, image, batch_size=10):
    device = sam.device
    predictor = SamMobilePredictor(sam)
    predictor.set_image(image)
    batch_boxes = np.split(boxes_xyxy, range(batch_size, len(boxes_xyxy), batch_size))
    batch_masks = []
